...
class TrainBase(object):
    ...
    def get_code(self, data_loader, length: int, feature_map):
        ...
        for image, text, label, index in tqdm(data_loader):
            ...
            index = index.numpy()
            image_hash = self.model.encode_image(image, feature_map)
            image_hash = torch.sign(image_hash)
            text_hash = self.model.encode_text(text, feature_map)
