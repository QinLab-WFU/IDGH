...
def pos_neg_mask_xy(labels_col, labels_row, batch_size):
    ...
    intersection = torch.matmul(labels_col.float(), labels_row.float().T)
    pos_mask = (intersection > 0)

    if labels_col is labels_row:
        identity_mask = torch.eye(batch_size, dtype=torch.bool, device=labels_col.device)
        pos_mask = pos_mask & (~identity_mask)

    neg_mask = (intersection == 0)
...
class MultiSimilarityLoss(nn.Module):
    ...
    def forward(self, inputs_col, targets_col, inputs_row=None):
        ...
        # with aug
        if self.batch_size != inputs_col.shape[0] and not self.all_anchor:
...
class ContrastiveLoss(nn.Module):
    ...
    def forward(self, inputs_col, inputs_row, inputs_col_u, inputs_row_u, targets_col):
        if inputs_row is None:
            inputs_row = inputs_col
        targets_row = targets_col
        if self.batch_size == inputs_col.shape[0] and not self.all_anchor:
